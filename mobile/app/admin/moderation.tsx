...
export default function AdminModerationScreen() {
  ...
  return (
    <ParallaxScrollView headerBackgroundColor={{ light: '#A1CEDC', dark: '#1D3D47' }} headerImage={null}>
      <ThemedView style={styles.titleContainer}>
        <ThemedText type="title">Admin Moderation</ThemedText>
      </ThemedView>
      ...
      <View style={styles.tabsContainer}>
        <TouchableOpacity
          ...
          disabled={loading}
        >
          <ThemedText style={[styles.tabText, tab === 'users' && styles.activeTabText]}>Users</ThemedText>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          disabled={loading}
        >
          <ThemedText style={[styles.tabText, tab === 'disputes' && styles.activeTabText]}>Disputes</ThemedText>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          disabled={loading}
        >
          <ThemedText style={[styles.tabText, tab === 'documents' && styles.activeTabText]}>Documents</ThemedText>
        </TouchableOpacity>
      </View>
      ...
      <ThemedView style={styles.stepContainer}>
        ...
        {tab === 'users' ? (
            ...
            {users.map((user) => (
              <View key={user.id} style={styles.userCard}>
                ...
                <View style={styles.buttonRow}>
                  ...
                  <TouchableOpacity style={[styles.actionButton, styles.verifyButton]} onPress={() => handleVerify(user)}>
                    <ThemedText style={styles.buttonText}>
          ...
          <>
            {disputes.map((d) => {
              const isResolved = d.status === 'resolved_valid' || d.status === 'resolved_invalid';
              ...
              return (
                <View key={d.id} style={styles.userCard}>
                  ...
                  <ThemedText style={styles.metaText}>Created: {new Date(d.createdAt).toLocaleString()}</ThemedText>
                  ...
                  {!isResolved ? (
                    <View style={styles.disputeButtonRow}>
                      <ThemedButton
                        ...
                        style={{ flex: 1 }}
                      />
                      <ThemedButton title="Valid" onPress={() => handleResolveDispute(d.id, 'resolved_valid')} style={{ flex: 1 }} />
                      <ThemedButton
                        title="Invalid"
                        variant="secondary"
          ...
          <>
                ...
                <View key={doc.id} style={styles.userCard}>
                  <View style={styles.userHeader}>
                    <View>
                      ...
                      <ThemedText style={styles.metaText}>User: {doc.userId}</ThemedText>
                    </View>
                  ...
                  <View style={styles.disputeButtonRow}>
                    ...
                    <ThemedButton
                      ...
                      variant="secondary"
                      onPress={() =>
                        handleReviewDocument(doc.id, 'rejected', 'Please re-upload a clearer document.')
                      }
                      style={{ flex: 1 }}
            ...
            {documents.length === 0 && !loading && <ThemedText>No documents found</ThemedText>}
          </>
        )}
      </ThemedView>
    </ParallaxScrollView>
